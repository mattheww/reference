...
If the path starts with `::` or an identifier, it is treated as if it began
with `crate`.

> **Note**: to make it possible to use a crate name in a `use` path, use an
...
> **Edition Differences**: Beginning in the 2018 edition, A `use` path which
> starts with `::` is resolved from the [extern prelude] instead of the crate
> root.
>
> Also, a `use` path which starts with an identifier is resolved in the same
> way as any other path, except that it is an error if the path has more than
> one possible resolution (unless they resolve to the same thing); 'inner'
> bindings do not shadow 'outer' ones.
>
